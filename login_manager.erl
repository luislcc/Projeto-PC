   ...
   receive
    ...
    {create_account,Pid,Username,Passwd} -> 
    	case maps:is_key(Username,Dbase) of
    		...
    		false -> Pid ! {ok,?MODULE}, loop(maps:put(Username,{Passwd,false},Dbase))
    	end;

    {close_account,Pid,Username,Passwd} ->
    	case maps:get(Username,Dbase,error) of
    		{Passwd,_} -> Pid ! {ok,?MODULE}, loop(maps:remove(Username,Dbase));
    		error -> Pid ! {invalid,?MODULE},loop(Dbase)
    	end;
    ...
    {login,Pid,Username,Passwd} ->
    	case maps:get(Username,Dbase,error) of
    		...
    		_ -> Pid ! {invalid,?MODULE},loop(Dbase)
    ...
    {online,Pid} -> 
    		Pid ! {[X || X <- maps:keys(Dbase), element(2,maps:get(X,Dbase))],?MODULE},
    		loop(Dbase)
